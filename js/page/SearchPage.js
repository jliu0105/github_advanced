...
class SearchPage extends Component<Props> {
    ...
    loadData(loadMore) {
        ...
        if (loadMore) {
            onLoadMoreSearch(++search.pageIndex, pageSize, search.items, this.favoriteDao, callback => {
                this.toast.show('No more content');
            });
        } else {
    ...
    onBackPress() {
        ...
        if (this.isKeyChange) {
            onLoadLanguage(FLAG_LANGUAGE.flag_key);//reload label
        }
        return true;
    }
    ...
    saveKey() {
        ...
        if (Utils.checkKeyIsExist(keys, key)) {
            this.toast.show(key + 'have existed');
        } else {
            key = {
                'path': key,
                'name': key,
            ...
            this.toast.show(key.name + 'Successfully saved');
        ...
        let bottomButton = showBottomButton ?
            ...
            >
                <View style={{justifyContent: 'center'}}>
                    <Text style={styles.title}>I'll get this</Text>
                </View>
