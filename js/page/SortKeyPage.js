...
class SortKeyPage extends Component<Props> {
    ...
    componentDidMount() {
        ...
        // if label in props is empty then get label locally
    ...
    static _keys(props, state) {
        // use checkedArray in state if it has one
        if (state && state.checkedArray && state.checkedArray.length) {
            return state.checkedArray;
        }
        //Otherwise get checkedArray from original data
    ...
    onSave(hasChecked) {
        if (!hasChecked) {
            // if no sorting then just return
