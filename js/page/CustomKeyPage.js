...
class CustomKeyPage extends Component<Props> {
    ...
    static _keys(props, original, state) {
        ...
        if (isRemoveKey && !original) {
            //if keys in state is null then get it from props
            return state && state.keys && state.keys.length !== 0 && state.keys || props.language[key].map(val => {
                return {//cannot change props directly, just copy one
                    ...val,
                    checked: false,
                };
            });
    ...
    render() {
        const {theme} = this.params;
        let title = this.isRemoveKey ? 'Remove Label' : 'Customize label';
